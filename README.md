...
- **Genuinely autonomous monitoring (keeperless).** A standalone `PriceFeedPoller` runs a **self-rescheduling Reactivity cron** that dispatches a JSON-API agent to fetch the **real** USDC price on-chain every cycle and writes it to a dedicated **USDC·live** asset — no off-chain keeper anywhere. This is what makes *"Sentinel detects depegs"* literal: the dashboard's **LIVE MONITOR** shows the real peg observed on-chain, and **USDC·live is real coverage anyone can buy** — a genuine depeg there autonomously fires the full pipeline (investigation reads Circle's real status sources) and pays its policyholders. The four other stables (USDC/USDT/DAI/FRAX) are **operator-controlled demo assets**, each wired to a payout class via the **DEMO CAUSE** switch (which re-points the issuer pages on-chain) so every matrix cell is demoable on demand.
