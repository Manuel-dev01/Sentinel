...
- **Consensus is 3-of-3 unanimity via `createAdvancedRequest`, not basic-request majority (2026-05-31).** The product thesis is "anything less than 3-of-3 and the payout does not sign," but a basic `createRequest` returns only the validators the platform happens to schedule (every prior spike got 2), and the original `_consensusResult` advanced on a 2-of-3 majority — undercutting the pitch. A gating spike (`script/spike-consensus.ts`, `SpikeJsonApi.fireAdvanced`) proved that `createAdvancedRequest(agentId, cb, sel, payload, subcommitteeSize=3, threshold=3, ConsensusType.Threshold, timeout=300)` reliably collects 3 byte-identical validator responses on testnet. So `_dispatch` now uses the advanced request with `SUBCOMMITTEE_SIZE = 3`/threshold 3 and `getAdvancedRequestDeposit`, and `_consensusResult` requires **full-committee unanimity** (every response `Success` + byte-identical, `responses.length == SUBCOMMITTEE_SIZE`). Gotcha: `timeout = 0` reverts on the platform; a non-zero timeout (300 s) is required. The audit screen's stamp reads `N/N CONSENSUS` only on unanimity. Locked by `test_majority_2of3_fails_under_unanimity`, `test_partial_2_responders_fails_under_unanimity`.
...
- **Multiple insured stables share one pool; the frontend selects per-stable (2026-05-31).** The Registry always supported many stables; `deploy.ts` now registers two (USDC + USDT) and the frontend reads the deploy-synced `STABLES` list with a localStorage-backed selected-stable context (`lib/stables.tsx`). The dashboard + coverage screens scope to the selected stable; the LP pool is shared capital, so it stays global but locks withdrawals if **any** stable has a live event. `/audit` is event-scoped (reads the event's own `stable`) so it needs no selector.
