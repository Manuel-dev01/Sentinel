...
| 0.4 | Oracle funded and armed | The Oracle holds at least 32 STT and owns subscription `3711687` | On Shannon Explorer, open the Oracle address and check the balance. | A balance under 32 STT means detection may stop firing. |
...
| 2.5 | Simulate depeg (demo stable) | Price drops to `$0.9200`, `800 bps · BREACH`, the strip reads `IN FLIGHT`, and the stepper begins | Select a DEMO stable, click Simulate Depeg, confirm in MetaMask. | Nothing happens after the tx confirms, indicating a price-oracle wiring or subscription issue. |
...
| 9.6 | Monitor longevity and calibration | The monitor runs until the spendable balance above 32 STT is exhausted, and live assets fire only on a real, sustained 2% depeg | If the poll count freezes, run `pnpm hardhat run script/fund-poller.ts --network somniaTestnet` to top up and `pnpm hardhat run script/rearm-poller.ts --network somniaTestnet` to re-arm. Confirm `registry.getConfig(liveAsset).depegThresholdBps` is 200. | A 50 bps threshold makes a live asset fire on normal market wobble and drains the Oracle. Run `script/calibrate-live.ts`. |
